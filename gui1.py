...
window.title("Tech Marvel Booking")
...
image_label.place(x=13, y=100, width=170, height=65)  # Same position as the button
#-------------------------------------------------------------------#
def open_gui2():
...
button_2.place(
    x=13, y=170, width=165, height=50
)
#--------------------------------------------------------------------------#
def open_gui4():
    subprocess.Popen(["python", r"C:\Users\somna\VScode\project\page1\build\gui2.py"])
    window.destroy()
...
def get_doctor_suggestion(symptom):
    try:
        ...
        query = "SELECT * FROM doctors WHERE symptoms LIKE %s"
        cursor.execute(query, ("%"+symptom+"%",))

        doctors = cursor.fetchall()

        if doctors:
            response_text = "Here are some recommended doctors:\n"
            for doctor in doctors:
                response_text += f"- {doctor['name']} ({doctor['specialization']}) - {doctor['location']}, Available: {doctor['availability']}, Symptoms: {doctor['symptoms']}\n"
